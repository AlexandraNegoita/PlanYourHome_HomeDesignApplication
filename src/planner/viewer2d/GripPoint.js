...
export class GripPoint extends PIXI.Graphics {
    ...
    model;
    walls = [];
    constructor(walls, board, model, lineSize, lineColor, snapMode) {
        super();
        this.walls = walls;
        this.board = board;
        this.model = model;
        ...
        if (snapMode)
            this.center.changeSnapMode(snapMode);
    }
    setup(stage, editMode) {
    ...
    drawTemporary(x, y) {
        let newPos = this.center.snapToPoint(this.board, x, y);
