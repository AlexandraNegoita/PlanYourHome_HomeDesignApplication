...
export class Room extends PIXI.Graphics {
    ...
    currentFurnitureType = 1;
    furniturePiece = new Furniture(this, this.model, this.spriteManager, this.currentFurnitureType);
    currentFurnitureRotation = 0;
    constructor(roomID, lineSize, lineColor, snapMode) {
        super();
    ...
    setup(model, board, spriteManager, snapMode) {
        this.model = model;
        this.board = board;
    ...
    handleKeyDown = (e) => {
        if (e.key === 'r' || e.key === 'R') {
            if (this.coords.mode == SnapMode.GRID) {
                if (this.currentFurnitureRotation % 8 == 0)
                    this.currentFurnitureRotation = 0;
                this.currentFurnitureRotation += Math.PI / 2;
            }
            else
                this.currentFurnitureRotation += Math.PI / 8;
            if (this.objectsContainer.children.includes(this.furniturePiece)) {
                this.furniturePiece.rotation = this.currentFurnitureRotation;
            }
        }
    };
    drawTemporaryFurniture(x, y) {
        if (this.objectsContainer.children.includes(this.furniturePiece))
            this.objectsContainer.removeChild(this.furniturePiece);
        let newPos;
        if (this.coords.mode == SnapMode.GRID) {
    ...
    createRoom(walls) {
        if (!walls || walls.length === 0 || !walls[0])
            return;
        this.clear();
        this.stroke({ width: this.lineTempWidth, color: this.lineTempColor });
