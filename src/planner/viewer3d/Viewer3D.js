...
export class Viewer3D {
    ...
    materials;
    currentMaterialCategory = "WALL";
    constructor(model, board, textures) {
    ...
    getTextures(textureArray) {
        const allPromises = [];
        textureArray.forEach((jsonMat) => {
            allPromises.push(new Promise((resolve, reject) => {
                new THREE.TextureLoader(this.loadingManager).load(jsonMat.path, function (texture) {
                    const loadedTexture = {
                        id: jsonMat.id,
                        for: jsonMat.for,
                        path: jsonMat.path,
    ...
    setup(fov, width, height, near, far, htmlElement) {
    ...
    setupConfigsUI() {
        ...
        if (shininess) {
            shininess.addEventListener("input", (e) => {
    ...
    updateCurrentMaterial(materialCategory) {
