...
export class Door {
    ...
    utils = new Utils();
    buildDoor(doorData, wallID, width, height, thickness, orientation, angle) {
        if (doorData.door.partOfWall == wallID) {
            let local1 = new THREE.Vector3(this.utils.calculateRatio(doorData.door.centerPoint.coordX), this.utils.calculateRatio(doorData.door.centerPoint.coordY) + this.utils.calculateRatio(height) * 0.6);
    ...
    buildDoorFrame(doorData, wallID, width, height, thickness, orientation, angle) {
        if (doorData.door.partOfWall == wallID) {
            let local1 = new THREE.Vector3(this.utils.calculateRatio(doorData.door.centerPoint.coordX), this.utils.calculateRatio(doorData.door.centerPoint.coordY) + this.utils.calculateRatio(height) * 0.6);
            ...
            if (orientation == 'vertical')
                doorFrameGeometry.rotateZ(Math.PI / 2);
            else if (orientation == 'diagonal')
                doorFrameGeometry.rotateZ(angle);
