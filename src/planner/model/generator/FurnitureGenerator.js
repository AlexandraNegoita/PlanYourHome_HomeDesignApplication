...
export class FurnitureGenerator {
    ...
    validatePlan(plan) {
        ...
        for (const roomNode of plan.rooms) {
            // @ts-ignore
            if (!roomNode.room.roomType || roomNode.room.roomType.trim() === "")
                return false;
        }
        return true;
    }
    ...
    async generate(plan) {
        ...
        if (!newPlan.objects)
            newPlan.objects = { windows: [], doors: [], furniture: [] };
        ...
        for (const roomNode of newPlan.rooms) {
            ...
            let roomPieces = furnitureAssets.filter((f) => f.roomType === type);
            if (roomPieces.length === 0) {
                roomPieces = furnitureAssets.filter((f) => f.roomType === "other");
            }
    ...
    async placeGroupOnGrid(wall, pieces, geom, plan, startId, roomID) {
    ...
    async placeGroupInCenter(pieces, centerX, centerY, plan, startId, roomID) {
        const pieceData = [];
        let totalWidth = 0;
        for (const piece of pieces) {
            const dims = await this.getDimensions(piece);
