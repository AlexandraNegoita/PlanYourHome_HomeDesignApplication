...
export class Generator {
    ...
    constructor(private config: GeneratorConfig, private board: Board) {
    ...
    private attemptGeneration(): Plan {
    ...
    // -----------------------------
    private divideSpace(initialSpace: any): RoomNode[] {
        let idCounter = 1; 
        const initialRoom: RoomNode = { id: idCounter++, ...initialSpace, wallIDs: [] };
        let rooms = [initialRoom];
        let splitting = true;

        while (splitting && rooms.length < this.config.rooms) {
            splitting = false;
            const nextRooms: RoomNode[] = [];
            ...
            for (const room of rooms) {
                ...
                if (aspectRatio > 1.5) splitVertically = true; 
                if (aspectRatio < 0.66) splitVertically = false; 
                ...
                if (splitVertically && canSplitV) {
                    ...
                    nextRooms.push(
                        { ...room, id: idCounter++, width: splitWidth, wallIDs: [] },
                        { ...room, id: idCounter++, x: snappedX, width: room.width - splitWidth, wallIDs: [] }
                    );
                    ...
                    nextRooms.push(
                        { ...room, id: idCounter++, height: splitHeight, wallIDs: [] },
                        { ...room, id: idCounter++, y: snappedY, height: room.height - splitHeight, wallIDs: [] }
                    );
                } else {
                    nextRooms.push(room);
                }
    ...
    // -----------------------------
    private generateAtomicWalls(rooms: RoomNode[]): WallNode[] {
        ...
        const addSegment = (x1: number, y1: number, x2: number, y2: number, roomID: number) => {
            ...
            if (wallMap.has(key)) {
                ...
                if (!existing.roomID.includes(roomID)) existing.roomID.push(roomID);
            } else {
                wallMap.set(key, {
        ...
        for (const r of rooms) {
            for (let i = 0; i < xs.length - 1; i++) {
                if (xs[i] >= r.x && xs[i+1] <= r.x + r.width) addSegment(xs[i], r.y, xs[i+1], r.y, r.id);
                if (xs[i] >= r.x && xs[i+1] <= r.x + r.width) addSegment(xs[i], r.y + r.height, xs[i+1], r.y + r.height, r.id);
            }
            for (let i = 0; i < ys.length - 1; i++) {
                if (ys[i] >= r.y && ys[i+1] <= r.y + r.height) addSegment(r.x, ys[i], r.x, ys[i+1], r.id);
                if (ys[i] >= r.y && ys[i+1] <= r.y + r.height) addSegment(r.x + r.width, ys[i], r.x + r.width, ys[i+1], r.id);
            }
        }
    ...
    private linkWalls(walls: WallNode[]) {
        const isSame = (p1: any, p2: any) => p1.coordX === p2.coordX && p1.coordY === p2.coordY;

        for (const wall of walls) {
            for (const other of walls) {
                if (wall.wallID === other.wallID) continue;
    ...
    // -----------------------------
    private generateRoof(walls: WallNode[], rooms: RoomNode[]): WallNode[] {
    ...
    // -----------------------------
    private generateDoorsAndWindows(walls: WallNode[], rooms: RoomNode[]) {
        const doors: any[] = [];
        const windows: any[] = [];
        let doorID = 1;     
        let windowID = 1;   
        ...
        const occupiedSpaces = new Map<number, { center: number, size: number }[]>();

        const getWallLength = (w: WallNode) => Math.hypot(w.endPoint.coordX - w.startPoint.coordX, w.endPoint.coordY - w.startPoint.coordY);

        const interiorWalls = walls.filter(w => w.roomID.length === 2);
        const exteriorWalls = walls.filter(w => w.roomID.length === 1);

        const connections = new Map<string, WallNode[]>();
        ...
        const potentialInternalConnections: { r1: number, r2: number, walls: WallNode[] }[] = [];
            ...
            if (isMandatory || isRandomExtra) {
                ...
                for (const w of conn.walls) {
                    ...
                    if (pos !== null) {
                        ...
                        if (isMandatory) union(root1, root2);
                        placed = true;
                        break; 
                    }
                }
    ...
    // -----------------------------
    private assignRoomTypes(rooms: RoomNode[]) {
        ...
        for (let i = sortedRooms.length - 1; i >= 0; i--) {
            if (sortedRooms[i].roomType) continue; // Skip if already a bathroom
        ...
        rooms.forEach(r => {
            if (!r.roomType) r.roomType = "other";
        });
    }

    // --- HELPER: FINDS A RANDOM, UNOCCUPIED GAP ON A WALL ---
    private findPlacementOnWall(
        wall: WallNode, 
        objSize: number, 
        cornerPad: number, 
        itemPad: number, 
        occupiedSpaces: Map<number, { center: number, size: number }[]>
    ): { coordX: number, coordY: number, distanceAlongWall: number } | null {
        ...
        for (const occ of existingObjects) {
            ...
            for (const r of validRanges) {
                if (blockMax <= r.min || blockMin >= r.max) {
                    ...
                    if (blockMin > r.min) nextRanges.push({ min: r.min, max: blockMin });
                    if (blockMax < r.max) nextRanges.push({ min: blockMax, max: r.max });
                }
            }
    ...
    private markWallOccupied(wallID: number, centerDist: number, size: number, occupiedSpaces: Map<number, any[]>) {
        if (!occupiedSpaces.has(wallID)) occupiedSpaces.set(wallID, []);
        occupiedSpaces.get(wallID)!.push({ center: centerDist, size: size });
    }

    private createObjectNode(type: "door" | "window", id: number, wall: WallNode, x: number, y: number) {
        if (type === "door") {
